"""Domain service for parsing and validating trading signals from text strings."""
...
from src.domain.exceptions.signal import InvalidSignalDataError, SignalParseError


class SignalParserDomainService:
    """Pure domain service for parsing raw Telegram signals into structured ParsedSignalDTO."""
    ...
    TIMEFRAME_PATTERN = re.compile(
        r"(?:^|\n)\s*(?:⏱\s*)?(?:TIMEFRAME|TF)[:\s]*([0-9]+[mhdwMHDW])\b",
        re.IGNORECASE,
    )
    ...
    def parse(self, raw_text: str, strict: bool = False) -> ParsedSignalDTO:
        ...
        timeframe = self._extract_timeframe(clean_text)
        pattern = self._extract_pattern(clean_text)
        notes = self._extract_notes(clean_text)
        ...
        # Extract explicit confidence score if present (e.g., "Confidence Score (AI): 70%")
        conf_match = re.search(r"Confidence(?:\s*Score)?(?:\s*\(AI\))?:\s*(\d+(?:\.\d+)?)\s*%?", raw_text, re.IGNORECASE)
        if conf_match:
    ...
    def _extract_timeframe(self, text: str) -> Optional[str]:
        """Extract signal timeframe string (e.g. 15m, 1H, 4h, 1D)."""
        match = self.TIMEFRAME_PATTERN.search(text)
        if match:
            return match.group(1).upper()
        return None

    def _extract_pattern(self, text: str) -> Optional[str]:
        """Extract chart pattern description (e.g. Ranging Channel, Falling Wedge)."""
        match = self.PATTERN_PATTERN.search(text)
        if match:
            return match.group(1).strip()
        return None

    def _extract_notes(self, text: str) -> Optional[str]:
        """Extract AI analysis note or reasoning if present."""
        match = self.NOTE_PATTERN.search(text)
        if match:
            return match.group(1).strip()
        return None
