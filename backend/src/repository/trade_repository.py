...
class TradeRepository:
    ...
    async def get_daily_risk(self, date_str: str) -> Optional[DailyRiskConfig]:
        """Mengambil konfigurasi risk harian berdasarkan tanggal (YYYY-MM-DD)."""
    ...
    async def create_daily_risk_snapshot(
        ...
        """Menyimpan snapshot saldo dan batasan risk harian (Cron jam 00:00 WIB)."""
    ...
    async def create_trade_with_risk(
        ...
        """Membuat record Trade baru sekaligus entri kalkulasi TradeRisk dalam 1 transaksi."""
        ...
        await self.session.flush()  # Ambil auto-generated trade.id
    ...
    async def update_trade_status(
        self, trade_id: int, status: str, opened_at: Optional[datetime] = None, closed_at: Optional[datetime] = None
    ) -> None:
        """Update status perjalanan trade (WAITING_ENTRY, OPEN, PARTIAL, CLOSED, CANCELLED)."""
    ...
    async def create_order(
        ...
        """Mencatat order baru yang dikirim ke Binance (ENTRY, TP1, SL, dll)."""
        order = Order(
            trade_id=trade_id,
    ...
    async def update_order_status(
        ...
        """Update status order dari WebSocket Stream Binance (FILLED, CANCELED, dll)."""
        values = {"status": status}
    ...
    async def record_execution(
        ...
        """Catat log partial fill / full fill dari Binance User Data Stream."""
        execution = Execution(
            order_id=order_id,
            trade_id=trade_id,
            price=price,
    ...
    async def log_event(self, trade_id: int, event_type: str, payload_json: Optional[str] = None) -> None:
        """Mencatat milestone trade event (SL_MOVED_TO_BEP, TP1, TRAILING_ENABLED, dll)."""
    ...
    async def save_summary(
        ...
        """Menyimpan ringkasan statistik performa setelah trade selesai (CLOSED)."""
    ...
    async def has_active_trade_for_symbol(self, symbol: str, side: Optional[str] = None) -> bool:
        """Mengecek apakah ada Trade aktif (WAITING_ENTRY, OPEN, PARTIAL) di DB untuk symbol & side terkait."""
        conditions = [
            Trade.symbol == symbol,
    ...
    async def get_active_trades(self) -> List[Trade]:
        """Mengambil semua trade yang berstatus WAITING_ENTRY, OPEN, atau PARTIAL."""
        stmt = select(Trade).where(Trade.status.in_(["WAITING_ENTRY", "OPEN", "PARTIAL"]))
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def get_performance_summary(self) -> dict:
        """Mengkalkulasi agregasi performa dari tabel trade_summary."""
        ...
        if not row or not row.total_trades:
            return {"total_trades": 0, "winning_trades": 0, "losing_trades": 0, "winrate": 0.0, "total_net_pnl": 0.0, "total_commission": 0.0}
    ...
    async def get_expired_waiting_trades(self, max_hours: int = 4) -> List[Trade]:
        """Mencari trade berstatus WAITING_ENTRY yang umurnya sudah lebih dari max_hours."""
