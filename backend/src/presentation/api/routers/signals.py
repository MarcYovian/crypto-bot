...
from src.presentation.api.deps import (
    get_current_user,
    get_signals_feed_use_case,
    get_manual_execute_signal_use_case,
    get_cache,
)
...
async def get_signals_feed(
    ...
    use_case: GetSignalsFeedUseCase = Depends(get_signals_feed_use_case),
    ...
    feed_dto = await use_case.execute(
        page=page,
        page_size=page_size,
        status=status,
    )
...
async def manual_execute_signal(
    ...
    account_id: int = Query(default=1, ge=1, description="Trading Account ID"),
    current_user: User = Depends(get_current_user),
    use_case: ManualExecuteSignalUseCase = Depends(get_manual_execute_signal_use_case),
    cache: AsyncInMemoryCache = Depends(get_cache),
) -> TradeExecutionResultResponseDTO:
