...
from src.presentation.api.deps import (
    ...
    get_equity_curve_use_case,
    get_cache,
)
from src.infrastructure.persistence.models.users import User
from src.presentation.api.schemas.analytics import AnalyticsSummaryDTO, EquityPointDTO
...
async def get_analytics_summary(
    ...
    response_dto = await use_case.execute(account_id=account_id)
...
async def get_equity_curve(
    ...
    current_user: User = Depends(get_current_user),
    use_case: GetEquityCurveUseCase = Depends(get_equity_curve_use_case),
    cache: AsyncInMemoryCache = Depends(get_cache),
) -> List[EquityPointDTO]:
    ...
    points = await use_case.execute(account_id=account_id, timeframe=timeframe)
