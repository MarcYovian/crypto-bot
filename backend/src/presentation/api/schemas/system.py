...
class BotStatusDTO(BaseSchema):
    ...
    scheduler_jobs_count: int = Field(default=8, description="Active background cron job count")
...
class TradingCredentialCreateRequest(BaseSchema):
    ...
    account_id: Optional[int] = Field(default=None, description="Optional target trading account ID")
