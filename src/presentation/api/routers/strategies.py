...
from src.presentation.api.schemas.master import StrategyDTO, StrategyUpdateRequest
...
async def update_strategy(
    ...
    use_case: UpdateStrategyUseCase = Depends(get_update_strategy_use_case),
    cache: AsyncInMemoryCache = Depends(get_cache),
) -> StrategyDTO:
    """Update TP allocation distributions and trailing/BEP triggers."""
    try:
        updated = await use_case.execute(id, payload)
